...
(function () {
  ...
  css.textContent = [
    ...
    // футер: кораблик везёт пиво Саше и Вите, они улыбаются при доставке
    '@keyframes crewSail{0%{transform:translate(398px,52px)}42%{transform:translate(176px,52px)}64%{transform:translate(176px,52px)}100%{transform:translate(398px,52px)}}',
    '.crew-ship{transform:translate(398px,52px);animation:crewSail 9s ease-in-out infinite}',
    '@keyframes crewSmile{0%,40%{opacity:0}46%,62%{opacity:1}68%,100%{opacity:0}}',
    '.crew-smile{animation:crewSmile 9s ease-in-out infinite}',
    '@keyframes crewMouthN{0%,40%{opacity:1}46%,62%{opacity:0}68%,100%{opacity:1}}',
    '.crew-mouth{animation:crewMouthN 9s ease-in-out infinite}',
    '@media(prefers-reduced-motion:reduce){.crew-ship{animation:none;transform:translate(176px,52px)}.crew-smile{opacity:1;animation:none}.crew-mouth{opacity:0;animation:none}}',
